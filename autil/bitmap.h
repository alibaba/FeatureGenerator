...
template<typename T>
static void POOL_DELETE_VECTOR_WITH_NUM(autil::mem_pool::PoolBase* pool, T *value, size_t num)
{
    if (value)
    {
...
#define POOL_COMPATIBLE_DELETE_VECTOR_WITH_NUM(pool, value, num)  \
    ...
    else { POOL_DELETE_VECTOR_WITH_NUM(pool, value, num); }       \
    value = NULL
...
public:
    ...
    {
        return UpperPack(bitCount, SLOT_SIZE) / BYTE_SLOT_NUM;
    }
